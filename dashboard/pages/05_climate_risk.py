...
def render(api: DashboardAPI, filters: dict) -> None:
    ...
    tab1, tab2, tab3, tab4 = st.tabs(["Risk Map", "District Ranking", "Risk Analysis", "SHAP Explanation"])
    ...
    with tab2:
        ...
        if risk_data_list:
            sorted_risks = sorted(risk_data_list, key=lambda x: x.get("composite_risk", 0), reverse=True)
            fig = comparison_bar(
                sorted_risks,
                x_key="district",
