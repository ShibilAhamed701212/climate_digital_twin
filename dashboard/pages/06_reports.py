...
def render(api: DashboardAPI, filters: dict) -> None:
    ...
    with tab1:
        ...
        for d in districts:
            ...
            with st.expander(f"{d}", expanded=True):
                ...
                with col1:
                    metric_card("Total Rainfall (YTD)", f"{summary.get('total_rainfall_ytd', 0):.1f} mm")
                with col2:
    ...
    with tab3:
        ...
        st.info("Charts can be exported individually using the camera icon in Plotly toolbar (top-right of each chart)")
    ...
    with tab4:
        ...
        if st.button("📄 Generate Report", type="primary", use_container_width=True):
            ...
            if "Risk Assessment" in include_sections:
                ...
                for d in districts:
                    summary = api.get_district_summary(d)
                    report_lines.append(f"- **{d}:** Risk Level = {summary.get('risk_level', 'N/A')}")
                report_lines.append("")
