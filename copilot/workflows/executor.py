    ...
    def execute(self, plan: Plan) -> list[ToolResult]:
        ...
        for step in plan.steps:
            if step.tool_name not in self._registry:
                results.append(ToolResult(tool_name=step.tool_name, success=False, error=f"Tool '{step.tool_name}' not available"))
                continue
            ...
            try:
                ...
                results.append(ToolResult(tool_name=step.tool_name, success=True, data=data, execution_time_ms=round(elapsed, 2)))
            except Exception as e:
                elapsed = (time.time() - start) * 1000
                results.append(ToolResult(tool_name=step.tool_name, success=False, error=str(e), execution_time_ms=round(elapsed, 2)))
        return results
