...
class DataDownloader:
    ...
    def _generate_grid(self, resolution: float, bounds: dict[str, float]) -> pd.DataFrame:
    ...
    def _generate_synthetic_rainfall(
        ...
        monsoon_phase = np.sin(2 * np.pi * np.arange(date_range_days + 1) / 365.0 - 0.5)
    ...
    def _generate_synthetic_temperature(
        ...
        for _, point in grid.iterrows():
            ...
            seasonal = amp * np.sin(2 * np.pi * np.arange(date_range_days + 1) / 365.0 - 0.5)
    ...
    def _save_synthetic_temperature(self, filename: str, is_max: bool) -> Path:
        ...
        df = self._generate_synthetic_temperature(self.start_date, self.end_date, grid, is_max)
        filepath = self.raw_dir / filename
        df.to_parquet(filepath.with_suffix(".parquet"))
        logger.info(
        ...
        if sources.get("primary") == "nasa_power":
            try:
                ...
                if dataset_key in self._nasa_power_cache:
                    ...
                    logger.info(
                        "NASA POWER data saved: %s (%d records)", parquet_path.name, len(df)
                    )
                    return parquet_path
                logger.warning("NASA POWER returned no data for %s", dataset_key)
            except Exception:
                logger.warning(
                    "NASA POWER download failed, falling back to synthetic", exc_info=True
                )
